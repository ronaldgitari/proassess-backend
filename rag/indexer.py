...
async def index_source(
    *,
    ...
    # Upsert to Chroma (embeds each chunk via OpenAI, then stores the vectors)
    chroma = get_chroma()
    async with pt.track_span("openai", f"embeddings · {settings.OPENAI_EMBEDDING_MODEL}", phase="index",
                             detail=f"embed {len(chunks)} chunks"):
        async with pt.track_span("chroma", "add_documents", phase="index",
                                 detail=f"store {len(chunks)} vectors"):
    ...
    async with pt.track_span("postgres", "INSERT document_chunks", phase="index",
                             detail=f"{len(db_chunks)} chunk rows"):
