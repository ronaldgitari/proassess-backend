...
### Ops / System Processes (`/api/v1/ops`) — `system_admin` only
| Method | Path | Description |
|---|---|---|
...
- **3 default groups** (mirror the existing roles): **Ops** = all 10 (≈ system_admin); **People & Culture** = all − `system.view` (≈ hr_admin); **Line Managers** = `kb.view`+`assessment.create/distribute/review`+`results.team` (≈ lm). Seeded per-org (`ensure_default_groups`; run `_seed_groups.py`).
...
- **Seeding:** `ensure_default_groups()` runs inside `seed.py` for new orgs (no standalone script).
